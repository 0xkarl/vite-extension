...
const object_multiplex_1 = __importDefault(
  require('@metamask/object-multiplex')
);
const safe_event_emitter_1 = __importDefault(
  require('@metamask/safe-event-emitter')
);
const fast_deep_equal_1 = __importDefault(require('fast-deep-equal'));
const vite_rpc_errors_1 = require('eth-rpc-errors');
...
class MetaMaskInpageProvider extends safe_event_emitter_1.default {
  ...
  constructor(
    ...
    if (
      ...
      throw new Error(
        messages_1.default.errors.invalidOptions(
          maxEventListeners,
          shouldSendMetadata
        )
      );
    ...
    jsonRpcConnection.events.on('notification', (payload) => {
      ...
      if (method === 'metamask_accountsChanged') {
        ...
        connectionStream.destroy(
          new Error(messages_1.default.errors.permanentlyDisconnected())
        );
      }
    });
    // miscellanea
  ...
  async request(args) {
    if (!args || typeof args !== 'object' || Array.isArray(args)) {
      throw vite_rpc_errors_1.ethErrors.rpc.invalidRequest({
  ...
  _rpcRequest(payload, callback) {
    ...
    if (!Array.isArray(payload)) {
      ...
      if (
        payload.method === 'vite_accounts' ||
        payload.method === 'vite_requestAccounts'
      ) {
  ...
  _handleDisconnect(isRecoverable, errorMessage) {
    if (
      ...
      let error;
      if (isRecoverable) {
        error = new vite_rpc_errors_1.EthereumRpcError(
  ...
  _handleAccountsChanged(accounts, isEthAccounts = false) {
    ...
    if (!fast_deep_equal_1.default(this._state.accounts, _accounts)) {
      // we should always have the correct accounts even before vite_accounts
      // returns
      if (isEthAccounts && this._state.accounts !== null) {
        this._log.error(
          `MetaMask: 'vite_accounts' unexpectedly updated accounts. Please report this bug.`,
          _accounts
        );
      }
  ...
  //====================
  /**
   * Equivalent to: vite.request('vite_requestAccounts')
   *
  ...
  enable() {
    ...
    return new Promise((resolve, reject) => {
      try {
        this._rpcRequest(
          { method: 'vite_requestAccounts', params: [] },
          utils_1.getRpcPromiseCallback(resolve, reject)
        );
      } catch (error) {
        reject(error);
  ...
  _sendSync(payload) {
    ...
    switch (payload.method) {
      ...
      default:
        throw new Error(
          messages_1.default.errors.unsupportedSync(payload.method)
        );
    }
    return {
      id: payload.id,
